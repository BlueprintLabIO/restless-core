...
This service gives one self-hosted Restless company verified human accounts,
email invitations and a people screen. It uses Better Auth and PostgreSQL.
Core continues to own company data, document permissions and Authority.
Signing in proves membership; it does not grant permission to spend money,
change company policy or control the company computer.

## Set up a new company

This setup path is for a brand-new company that has not been used through local
owner entry. It does not convert an existing local-owner company: authenticated
people receive durable human Actor identities, so private documents owned by the
local `owner` Actor do not transfer automatically.
...
`accounts.example.com` and `work.example.com`. The Core company image must be
available by immutable OCI digest.

1. Create the new company through the normal local setup and wait for Core to
   finish initializing it. Do not create documents or otherwise use the company
   through the local-owner cockpit. Stop Core before continuing. Keep this new
   company's state, resource namespace, port offset and profile settings when
   changing its entry configuration; do not initialize another company for
   network entry.
...
3. Install the service dependencies and generate its configuration. Replace the
   ...
   ```sh
   cd services/self-hosted-identity
...
4. Start the account service under your process supervisor:
   ...
   able to fetch the accounts origin's `/.well-known/jwks.json`; the accounts
...
6. Open the accounts origin. Create the account matching `--owner-email`, verify
   ...
   from **People**. They create and verify their own account using the invited
   email address, accept the invitation and select **Open company**.
...
Members can enter the company's collaboration surfaces. Administrators can also
invite and remove members; only the owner can remove an administrator. Company
Authority is managed separately inside Core. The owner and the currently signed-in
person cannot be removed from this screen.

Removing someone asks Core to revoke their existing sessions and open document
connections before reporting success. If Core is unavailable, the screen shows
**Removal pending** and **Retry removal**. New entry is blocked immediately and
stays blocked through account-service restarts. Retry when Core returns to finish
ending access. Existing Core sessions are not claimed to be revoked until Core
confirms the removal. Past contributions retain their attribution.
