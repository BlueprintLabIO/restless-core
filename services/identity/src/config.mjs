...
/**
 * The cockpit calls the membership admin API with the account session cookie,
 * which browsers send only to a same-site origin. Without the Public Suffix
 * List this is an approximation: both hosts must share a parent domain of at
 * least two labels. Loopback development origins are exempt; they exercise
...
export function validateConfig(value) {
  ...
  if (!sameSite(account, core))
    throw Error(
      "The account and Core origins must share a parent domain, such as accounts.example.com and work.example.com",
    );
