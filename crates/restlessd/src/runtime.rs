...
/// Build a new company's config from the four things the owner actually says.
///
/// This **writes nothing**. `company-create` is the single write path — it also
/// initialises the company in Authority, and a second creation path that saved
/// a config directly would produce a company Authority had never heard of. So
/// this validates and constructs; the command persists.
///
...
/// as the owner from its first minute. Here that is structural rather than
/// remembered — the fields are simply not reachable from this signature.
pub fn new_config(
    name: &str,
    mission: &str,
    model: &str,
...
mod create_tests {
    ...
    fn a_name_that_cannot_be_a_schema_is_refused() {
        for bad in ["Foo Bar", "9lives", "drop;table", "", "a-dash", "Café"] {
            ...
            assert!(
                new_config(bad, "", "zai/glm-5.2", None).is_err(),
                "{bad:?} must not produce a config either"
            );
        }
    ...
    fn a_new_company_starts_with_no_reach_into_the_world() {
        let config = new_config("fresh", "a bakery", "zai/glm-5.2", Some(25.0)).expect("create");
        assert!(config.providers.is_empty(), "no provider may be preset");
        assert!(config.credentials.is_empty(), "no credential may be preset");
        assert!(
            config.approved_parties.is_empty(),
            "no party may be pre-approved"
        );
    ...
    fn the_model_must_name_its_provider() {
        assert!(new_config("fresh", "", "company-general-v1", None).is_err());
        assert!(new_config("fresh", "", "", None).is_err());
    }

    // Not tested here: refusing to create over an existing company. That check
    // moved to the `company-create` dispatch arm when it became the single
    // write path, and it is tested there — `api::tests::creating_over_an_
    // existing_company_is_refused` — because a test that no longer covers the
    // code that enforces the rule is worse than no test.
}
