...
impl AuthorityStore {
    /// A store whose pool is never connected, for tests that exercise transport
    /// rather than storage. `connect_lazy` dials on first query, so this builds
    /// without Postgres and fails fast — inside the envelope — if a test reaches
    /// for a record it should not have needed.
    #[cfg(test)]
    pub(crate) fn lazy(database_url: &str) -> Self {
        Self {
