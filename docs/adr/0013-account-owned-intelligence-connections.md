...
On 26 September, the local cockpit showed the opening-message flow instead of the separate name-and-purpose form. Company settings offered individual copy actions for name, purpose, quality bar, model choices and limits; a purpose copy preview was inspected and cancelled without changing either company. Account connections now appear before company-only sign-ins in Company → Intelligence. The local service served the updated page, with `no-store` for the shell and service worker and long-lived caching only for fingerprinted assets. This local UI observation does not establish that Cloud's first-entry handoff or a two-tenant provider grant works against a real released Core plane.
