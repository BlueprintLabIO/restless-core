...
| **SPA posture** (prior control plane, 2 Aug state) | Calm main surface + right-hand exec chat + tab strip; outcomes/decisions/effects first. | Live product; matches the target posture. | **Lifted — see `web/`.** Design layer, primitives, composed surfaces and every page, rendering from fixtures. Carries no truth: reads come from `$lib/fixtures`, writes are null callback props. Wiring it means swapping the fixture for an OrgIntel read client and passing real write callbacks. |
