...
| [x] | **S05-T4 · The OrgIntel → owner-surface type seam, generated** | OrgIntel | `web/` is TypeScript and the read model is Rust, both hand-written. A renamed field fails nowhere — no compiler, no test, no runtime error; the owner reads a blank cell and cannot tell absence from breakage | — |
...
T4 landed ahead of T1/T2 because both add owner-surface reads over the same rows, and the guard is
cheapest to install before there are two consumers to reconcile. It is **not a slice under §16.2** —
it produces no artifact, decision or external outcome, and does not count toward the success contract.
...
## A placement finding, from building T4

Investigating T4 surfaced something larger than the ticket, recorded here because it shapes tickets we
have not written yet. **It is a finding, not a sprint-05 commitment.**

The question that prompted it was whether Rust was the wrong choice for an experimental-stage product.
The evidence says no, and says the real problem is elsewhere. Three sprints of `Evidence (observed
friction)` columns name zero language, compile-time or refactoring frictions; incremental `cargo check`
on the largest file is 1.34s. The frictions we recorded were conceptual — six wakes on a market that
did not exist, specs drifting ahead of code, a harness built to measure an absence.

**The boundary follows the layer diagram. It should follow iteration rate.**

| Tier | Changes | Should live as | Lives now as |
|---|---|---|---|
| Cold — identity, credentials, budgets, effects, receipts, process supervision, schema | monthly | compiled, strict | Rust ✓ |
| Warm — planner, allocator, critic router, stagnation detector, health signals | weekly | out-of-process, replaceable | Rust ✗ |
| Hot — prompts, roles, briefs, playbooks, context templates | hourly, *by the Exec itself* | files in the company runtime | Rust ✗ |

`ARCHITECTURE.md` §4.5 already requires this — *"the stable coordination core should not contain every
theory of management"* — and the code does not honour it. Two concrete misplacements:

- **`context.rs:120-200`.** The Exec's operating doctrine — how it delegates, when to spawn a critic,
  how it weighs receipts against its own journal — is ~80 lines of prose in a `format!` literal.
  Changing how the company *thinks* requires rebuilding a binary. `company.toml`'s `mission` and
  `companies/*/simulators/*.md` already prove the file-shaped version works; doctrine just never went
  through that seam.
- **`health.rs:343-410`.** `organisational()` fetches every commitment and every effect event, counts
  them into a `BTreeMap`, and compares against `EFFORT_WITHOUT_OUTPUT_USD = 5.0` and
  `REPEATED_FAILURE_THRESHOLD = 3`. Those are SQL aggregates and a config table, executed in Rust with
  the thresholds compiled in — under a spec section labelled *Product hypothesis*, i.e. explicitly
  expected to change.

**The recommendation is not one language.** The flexible tier is three different problem shapes
(LLM_CURE frame 2 — classify before choosing the tool):

- **Judgement** (planner, team designer, critic router, historian, curator) → **no programming
  language at all.** Markdown plus the `restless` CLI as the tool surface. Encoding these as code in
  any language is what CLAUDE.md forbids: *do not replace intelligence with static flows.*
- **Deterministic policy over relational facts** (health signals, stagnation, attention cost) →
  **declarative rules.** The pragmatic form is Postgres views: the facts are already there, thresholds
  become rows, and changing a signal is a migration rather than a recompile. If rules ever go genuinely
  recursive, that is where real Datalog earns its place — **not before**, per §16.1.
- **Imperative glue** (module runners, schedulers, adapters) → **TypeScript**, solely because it does
  not raise the language count: we already own it in `web/`, and after T4 it shares generated types
  with Rust.

Smallest first move, when a sprint has room: evict `health::organisational` to Postgres views. ~70
lines, already written as aggregation, already labelled a hypothesis.
...
- Does the placement finding earn a ticket this sprint, or does it wait? The `health.rs` eviction is
  small and self-contained; the `context.rs` doctrine move is larger and touches every wake, so it
  should not land mid-run while T3 is attempting real outreach.
