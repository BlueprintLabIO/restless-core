...
const beaconStart = await page.evaluate(() => {
  ...
  return {
    ...
    scale: [beacon.scale.x, beacon.scale.y, beacon.scale.z],
    rotation: [beacon.rotation.x, beacon.rotation.y, beacon.rotation.z],
  };
});
ok('the existing cairn exposes a rendered beacon review handle', !!beaconStart && beaconStart.inScene && beaconStart.visible, JSON.stringify(beaconStart));
ok(
...
const beaconMotion = beaconStart && (
  beaconLater?.scale.some((value, index) => Math.abs(value - beaconStart.scale[index]) > 0.005)
  || beaconLater?.rotation.some((value, index) => Math.abs(value - beaconStart.rotation[index]) > 0.005)
);
ok('the in-world beacon is softly animated', !!beaconMotion, JSON.stringify({ before: beaconStart?.scale, after: beaconLater?.scale }));
...
ok('the marker handles arrival at the cairn', !!arrived && /arrived|0\s*m/i.test(arrived.text), arrived?.text || 'missing');
