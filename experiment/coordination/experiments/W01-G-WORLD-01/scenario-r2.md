# Owner outcome — one playable Prism Cavern expedition (r2)
...
surface with a `prismSnapshot()` function that returns current serialisable truth in this exact shape:

```js
{
  biome: 'basin' | 'cavern',
  entrance: { present: boolean, visible: boolean },
  room: {
    present: boolean,
    visible: boolean,
    enclosingForms: number,
    crystals: number,
  ...
  console: { present: boolean, visible: boolean },
  returnPortal: { present: boolean, visible: boolean },
  encounter: {
    present: boolean,
    visible: boolean,
    speciesId: 'nullix',
  ...
  positions: {
    ...
    encounter: { x: number, z: number },
    return: { x: number, z: number },
  },
  objective: string,
  status: string,
  basinActorsVisible: boolean,
}
```

Presence, counts, material roles, positions and flags must be derived from the same runtime objects and
DOM the player sees, not duplicated constants that can drift. `bridge.text` is the persistent
non-colour statement of its current traversal meaning. `encounter.beyondGate` states authored spatial
truth, not merely whether the bridge is powered.

Do not add a test-only mutation API. The evaluator will choose a starter, position the existing player
from `positions`, switch ordinary team state, and dispatch the documented `R`/`F` keys through the live
page.
