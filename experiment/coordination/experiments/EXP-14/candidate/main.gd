...
func _build_world() -> void:
	...
	hud_banner.position = Vector2(130, 185)
	hud_banner.size = Vector2(1020, 118)
	hud_banner.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	hud_banner.add_theme_font_size_override("font_size", 28)
	var banner_panel := StyleBoxFlat.new()
	banner_panel.bg_color = Color(0.16, 0.02, 0.02, 0.92)
	banner_panel.border_color = Color(1.0, 0.35, 0.35, 1.0)
	banner_panel.set_border_width_all(3)
	banner_panel.corner_radius_top_left = 8
	banner_panel.corner_radius_top_right = 8
	banner_panel.corner_radius_bottom_left = 8
	banner_panel.corner_radius_bottom_right = 8
	banner_panel.content_margin_left = 20
	banner_panel.content_margin_top = 14
	hud_banner.add_theme_stylebox_override("normal", banner_panel)
...
func _feedback(kind: String, msg: String, shot: String) -> void:
	...
	if hud_banner:
		hud_banner.text = "ACTION BLOCKED\n" + msg if kind == "reject" else msg
		...
		banner_until_ms = Time.get_ticks_msec() + (6000 if kind == "reject" else 3500)
	if shot != "":
...
func _request_unload() -> void:
	...
	if journey_stage != JourneyStage.EXITED_AT_DESTINATION:
		...
		_send_feedback(caller, "reject", "ON-FOOT UNLOAD BLOCKED — drive the loaded truck to 40 m, then exit there", "ev_reject_journey")
		return
...
func _request_seat(enter: bool) -> void:
	...
	if enter:
		...
		if seat_occupant != caller:
			...
			return
		# At ARRIVED_SEATED the current seat occupant is necessarily the player
		# whose loaded drive reached the snapped endpoint.  Requiring the cached
		# journey_driver id here made a visibly valid client exit fail after state
		# replication timing, leaving the route-end HUD in the pre-delivery state.
